...
sw_split_name <- starwars %>% # Start with starwars
  separate(col = name, # Separate the name column...
           into = c("name_1","name_2"), sep = "-") # Into two columns '' & 'name_1 and name_2'
...
gasol_hookshots <- lakers %>% 
  ...
  mutate(date_updated = ymd(date)) %>% 
  mutate(month_updated = month(date_updated)) %>% 

  group_by(month_updated) %>% 
  count(month_updated)
...
storms_danger <- storms %>%
  mutate(danger = case_when(status == "hurricane" ~ "high", status = TRUE ~ "moderate"))
